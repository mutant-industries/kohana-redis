...
/**
 * Credis_Client (a fork of Redisent), a lightweight Redis PHP standalone client and phpredis wrapper
 *
...
class Credis_Client {
     ...
     * @throws Credis_Exception
     */
    public static function instance()
    {
        if (Credis_Client::$instance === null)
        {
            $config = Kohana::$config->load('redis');

            $instance = new Credis_Client($config->get('host'), $config->get('port'), $config->get('timeout'), $config->get('persistent', ''));

            if ($config->get('force_standalone', false))
            {
                $instance->forceStandalone();
            }

            if ($retries = $config->get('connect_retries', self::DEFAULT_CONNECT_RETRIES))
            {
                $instance->setMaxConnectRetries($retries);
            }

            if ($timeout = $config->get('read_timeout', self::DEFAULT_CONNECT_TIMEOUT))
            {
                $instance->setReadTimeout((float) $timeout);
            }

            if ($config->get('password'))
            {
                if ( ! $instance->auth($config->get('password')))
                {
                    throw new Credis_Exception('Unable to authenticate with the redis server.');
                }
            }

            // Always select database on startup in case persistent connection is re-used by other code
            if ( ! $instance->select((int) $config->get('database', 0)))
            {
                throw new Credis_Exception('The redis database could not be selected.');
            }

            Credis_Client::$instance = $instance;
        }

        return Credis_Client::$instance;
    }
    ...
    public function forceStandalone()
    {
        if ($this->connected)
        {
            throw new Credis_Exception('Cannot force Credis_Client to use standalone PHP driver after a connection has already been established.');
        }
    ...
    protected function convertHost()
    {
        if (preg_match('#^(tcp|unix)://(.*)$#', $this->host, $matches))
        {
            if ($matches[1] == 'tcp')
            {
                ...
                {
                    throw new Credis_Exception('Invalid host format; expected tcp://host[:port][/persistence_identifier]');
                }
                $this->host = $matches[1];
            ...
            else
            {
                ...
                {
                    throw new Credis_Exception('Invalid unix socket format; expected unix:///path/to/redis.sock');
                }
            }
        }
    ...
    public function connect()
    {
        ...
        if ($this->standalone)
        {
            ...
            if ($this->persistent)
            {
                if ($this->port === NULL)
                { // Unix socket
                    throw new Credis_Exception('Persistent connections to UNIX sockets are not supported in standalone mode.');
                }
        ...
        if ( ! $result)
        {
            ...
            throw new Credis_Exception("Connection to Redis failed after $failures failures." . (isset($errno) && isset($errstr) ? "Last Error : ({$errno}) {$errstr}" : ""));
        }
    ...
    public function setReadTimeout($timeout)
    {
        if ($timeout < -1)
        {
            throw new Credis_Exception('Timeout values less than -1 are not accepted.');
        }
    ...
    public function pSubscribe($patterns, $callback)
    {
        ...
        while ($patternCount--)
        {
            ...
            if (!$status)
            {
                throw new Credis_Exception('Invalid pSubscribe response.');
            }
        }
        try
        {
            while (1)
            {
                ...
                if ($type != 'pmessage')
                {
                    throw new Credis_Exception('Received non-pmessage reply.');
                }
        ...
        {
            if ($e->getCode() == Credis_Exception::CODE_TIMED_OUT)
            {
                try
                {
    ...
    public function subscribe($channels, $callback)
    {
        ...
        while ($channelCount--)
        {
            ...
            if (!$status)
            {
                throw new Credis_Exception('Invalid subscribe response.');
            }
        }
        try
        {
            while (1)
            {
                ...
                if ($type != 'message')
                {
                    throw new Credis_Exception('Received non-message reply.');
                }
    ...
    public function __call($name, $args)
    {
        ...
        if ($this->standalone)
        {
            ...
            if ($this->usePipeline)
            {
                if ($name == 'pipeline')
                {
                    throw new Credis_Exception('A pipeline is already in use and only one pipeline is supported.');
                } else if ($name == 'exec')
                {
                    if ($this->isMulti)
                    {
        ...
        else
        {
            ...
            catch (RedisException $e)
            {
                ...
                throw new Credis_Exception($e->getMessage(), $code, $e);
            }
            ...
            switch ($name)
            {
                ...
                case 'script':
                    ...
                    {
                        throw new Credis_Exception($error);
                    }
                    break;
                default:
                    $error = $this->redis->getLastError();
    ...
    protected function write_command($command)
    {
        ...
        if (feof($this->redis))
        {
            ...
            if (($this->isMulti && !$this->usePipeline) || $this->isWatching)
            {
                ...
                throw new Credis_Exception('Lost connection to Redis server during watch or transaction.');
            }
            $this->connected = FALSE;
            $this->connect();
        ...
        for ($written = 0; $written < $commandLen; $written += $fwrite)
        {
            ...
            {
                $this->connected = FALSE;
                throw new Credis_Exception('Failed to write entire command to stream');
            }
        }
    }
    ...
    protected function read_reply($name = '')
    {
        ...
        if ($reply === FALSE)
        {
            ...
            if ($info['timed_out'])
            {
                throw new Credis_Exception('Read operation timed out.', Credis_Exception::CODE_TIMED_OUT);
            }
            else
            {
                $this->connected = FALSE;
                throw new Credis_Exception('Lost connection to Redis server.', Credis_Exception::CODE_DISCONNECTED);
            }
        }
        ...
        switch ($replyType)
        {
            ...
            case '-':
                ...
                {
                    throw new Credis_Exception(substr($reply, 0, 4) == '-ERR' ? substr($reply, 5) : substr($reply, 1));
                }
                break;
            /* Inline reply */
            case '+':
            ...
            case '$':
                ...
                {
                    $this->connected = FALSE;
                    throw new Credis_Exception('Error reading reply.');
                }
            ...
            case ':':
                ...
                break;
            default:
                throw new Credis_Exception('Invalid response: ' . print_r($reply, TRUE));
                break;
        }
